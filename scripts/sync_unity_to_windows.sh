...
# sync_unity_to_windows.sh
# Linux UnityApp mirror -> Windows Unity source (one-way, no delete by default).

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"

SRC_ROOT="${REPO_ROOT}/UnityApp"
DST_ROOT="${UNITY_WIN_SRC:-/mnt/c/Projects/Project_Guitar/GuitarPracticeUnity}"
...
if [[ ! -d "${DST_ROOT}" ]]; then
  echo "[sync] destination path does not exist: ${DST_ROOT}"
  exit 1
fi
...
echo "[sync] policy=one-way linux->windows mirror, add/update only (no delete)"
