...
| 작업 | 담당 에이전트 | 상태 |
|---|---|---|
| Unity UI 패널, 운지 표시, 코칭 화면, DTO 정의 | `fingering-coach-agent` | ✅ MVP 활성 |
| ML/휴리스틱 추천 로직, 코드 인식 | `chord-recognizer-agent` | ⏸ Phase 4 예정 |
| 하드웨어 커맨드 프로토콜 & 보안 | `hardware-bridge-agent` | ⏸ Phase 5 예정 |

---

## 오케스트레이터 행동 규칙

서브에이전트에게 작업을 위임할 때 아래 규칙을 반드시 따른다.

### 위임 전
- 관련 파일을 직접 Read한 뒤 위임한다. 파일 내용을 모르는 채로 에이전트에게 넘기지 않는다.
- 위임 프롬프트에 현재 파일의 실제 코드를 포함시킨다. 에이전트가 추측으로 작업하지 않도록 한다.
- 이 프로젝트의 확장 시나리오를 프롬프트에 명시한다:
  - 곡 추가 → 코드 진행 탐색 시 동일한 애니메이션 API 재사용
  - 마이크 코드 인식 → 인식된 코드를 HighlightChord/TransitionToChord로 표시
  - 하드웨어 LED → FretMarker 상태를 LED 브릿지에 그대로 미러링

### 위임 후 (필수)
- 에이전트가 작성한 **모든 파일을 직접 Read**한다. 에이전트 완료 메시지만 보고 끝내지 않는다.
- Unity C# 파일이 수정된 경우 반드시 `recompile_scripts`를 실행하고 에러를 직접 확인한다.
- 에이전트 결과물의 문제를 발견하면 직접 수정한다. 재위임 루프를 만들지 않는다.
- 에이전트가 놓친 부분(예: C# 예약어 충돌, 기존 API 호환성, 씬 참조 연결)을 보완한다.

### 완료 조건
아래를 모두 확인한 뒤에야 작업을 완료로 처리한다:
1. 수정된 파일을 직접 읽고 로직이 올바른지 확인
2. Unity 스크립트는 컴파일 에러 0개 확인
3. 기존 public API(HighlightChord, SetActive 등)가 깨지지 않았는지 확인
4. 미래 확장 시나리오에서 이 코드가 재사용 가능한지 한 줄이라도 코멘트
